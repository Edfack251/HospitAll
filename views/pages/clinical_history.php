...
use App\Controllers\ClinicalHistoryController;
...
if (isset($_GET['id']) && !$patient_id) {
    UrlHelper::redirect('clinical_history');
}
...
if ($patient_id) {
    ...
    $labs = $data['labs'];
    $episodes = (new \App\Services\ClinicalEpisodeService($pdo))->fetchAllByPatient($patient_id);
} else {
                ...
                <?php foreach ($patients as $p): ?>
                    <tr class="border-b hover:bg-gray-50 transition-colors">
                        ...
                        <td class="py-4 text-right">
                            <a href="<?php echo App\Helpers\UrlHelper::url('clinical_history'); ?>?id=<?php echo $p['id']; ?>"
...
<?php else: ?>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-4 gap-8">
        ...
        <div class="lg:col-span-3 space-y-8">
            ...
            <!-- Tab: Episodios -->
            <div id="tab-episodios" class="history-content-tab space-y-6">
                <?php if (empty($episodes)): ?>
                    <div class="bg-white p-12 rounded-2xl border border-dashed border-gray-200 text-center">
                        <p class="text-gray-400 italic">No existen episodios clínicos registrados para este paciente.</p>
                    </div>
                <?php else: ?>
                            ...
                            <div class="bg-blue-50 px-6 py-4 flex justify-between items-center border-b border-blue-100">
                                <div>
                                    <h4 class="text-lg font-bold text-blue-900">
                                        <?php echo htmlspecialchars($ep['descripcion_problema']); ?>
                                    </h4>
                                    <p class="text-xs text-blue-700 mt-1">Inicio:
                                        <?php echo date('d/m/Y', strtotime($ep['fecha_inicio'])); ?>
                                        <?php if ($ep['fecha_cierre'])
                                            echo ' | Cierre: ' . date('d/m/Y', strtotime($ep['fecha_cierre'])); ?>
                                    </p>
                                </div>
                                <span
                                    class="px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider <?php echo $ep['estado'] === 'Abierto' ? 'bg-green-100 text-green-700' : 'bg-gray-200 text-gray-700'; ?>">
                                    <?php echo $ep['estado']; ?>
                                </span>
                            </div>

                            <div class="p-6">
                                <?php if (empty($ep['timeline'])): ?>
                                    <p class="text-sm text-gray-400 italic">No hay registros asociados a este episodio.</p>
                                <?php else: ?>
                                    <div class="relative border-l-2 border-blue-200 ml-3 space-y-6">
                                        <?php foreach ($ep['timeline'] as $item): ?>
                                            <div class="mb-8 pl-6 relative">
                                                <div
                                                    class="absolute w-4 h-4 bg-blue-500 rounded-full border-4 border-white -left-[9px] top-1">
                                                </div>
                                                <div class="text-xs font-bold text-blue-500 mb-1">
                                                    <?php echo date('d/m/Y', strtotime($item['fecha'])) . ' ' . date('H:i', strtotime($item['hora'])); ?>
                                                </div>
                                                <div class="bg-gray-50 p-4 rounded-xl border border-gray-100">
                                                    <h5 class="font-bold text-gray-800 text-sm mb-1">Cita con Dr.
                                                        <?php echo htmlspecialchars($item['medico_nombre'] . ' ' . $item['medico_apellido']); ?>
                                                    </h5>

                                                    <?php if (!empty($item['atencion'])): ?>
                                                        <div class="mt-2 text-sm text-gray-600">
                                                            <strong class="text-gray-700">Diagnóstico:</strong>
                                                            <p class="mb-1">
                                                                <?php echo nl2br(htmlspecialchars($item['atencion']['diagnostico'])); ?>
                                                            </p>
                                                            <strong class="text-gray-700">Tratamiento:</strong>
                                                            <p><?php echo nl2br(htmlspecialchars($item['atencion']['tratamiento'])); ?></p>
                                                        </div>
                                                    <?php else: ?>
                                                        <p class="mt-2 text-sm text-gray-500 italic">Cita
                                                            <?php echo strtolower($item['estado']); ?>
                                                        </p>
                                                    <?php endif; ?>

                                                    <?php if (!empty($item['laboratorios'])): ?>
                                                        <div class="mt-3 pt-3 border-t border-gray-200">
                                                            <strong class="text-xs text-gray-500 uppercase">Órdenes de Laboratorio:</strong>
                                                            <ul class="list-disc pl-5 mt-1">
                                                                <?php foreach ($item['laboratorios'] as $lab): ?>
                                                                    <li class="text-sm text-gray-700">
                                                                        <?php echo htmlspecialchars($lab['descripcion']); ?>
                                                                        (<?php echo $lab['estado']; ?>)
                                                                    </li>
                                                                <?php endforeach; ?>
                                                            </ul>
            ...
            <!-- Tab: Consultas -->
            <div id="tab-consultas" class="history-content-tab hidden space-y-6">
                            ...
                            <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <?php if ($h['observaciones']): ?>
                                    <div class="md:col-span-2">
                                        <?php if ($h['adenda_de_id']): ?>
                                            <h6 class="text-xs font-bold text-blue-500 uppercase mb-2">Adenda Clínica / Corrección Oficial
                                            </h6>
                                            <p class="text-sm text-blue-800 italic bg-blue-50 p-4 border-l-4 border-blue-400 rounded-r-xl">
