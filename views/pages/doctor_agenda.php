...
try {
    if ($medico_id) {
        ...
        // Próximas citas (Listado) - incluye hoy y futuras, solo no atendidas
        $stmtProx = $pdo->prepare("SELECT c.*, p.nombre as paciente_nombre, p.apellido as paciente_apellido 
                                   FROM citas c 
                                   JOIN pacientes p ON c.paciente_id = p.id AND p.deleted_at IS NULL 
                                   WHERE c.medico_id = ? AND c.estado IN ('Programada', 'Confirmada', 'En espera') 
                                   AND (c.fecha > CURDATE() OR c.fecha = CURDATE())
        ...
        foreach ($dbEvents as $e) {
            ...
            $events[] = [
                ...
                'url' => \App\Helpers\UrlHelper::url('patient_portal') . '?patient_id=' . $e['paciente_id'],
                'color' => $color
            ];
        }
    }
                    ...
                    <?php foreach ($proximasCitas as $c): ?>
                        <div class="p-3 border rounded-xl hover:border-[#007BFF] transition-all bg-gray-50/50">
                            ...
                            <?php if (in_array($c['estado'], ['Programada', 'Confirmada', 'En espera'])): ?>
                                <a href="<?php echo \App\Helpers\UrlHelper::url('appointments_attend'); ?>?id=<?php echo $c['id']; ?>"
