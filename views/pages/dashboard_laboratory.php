...
AuthHelper::checkRole(['tecnico_laboratorio', 'administrador']);
...
$data = $controller->getTecnicoLaboratorioData();
...
$pageTitle = 'Dashboard Laboratorio - HospitAll';
$activePage = 'dashboard_laboratory';
$headerTitle = 'Panel de Laboratorio';
...
<?php
function estadoBadgeClassLab($estado) {
    switch ($estado) {
...
<div class="mb-8 p-4 rounded-xl shadow-sm border border-l-4 border-l-orange-500 bg-orange-50 text-orange-800 flex items-center justify-between">
    <div>
        ...
        <p class="text-sm opacity-90 mt-1">
            <?php if ($existe_orden_walkin): ?>
                Se ha encontrado una orden de laboratorio pendiente en la tabla de abajo. Por favor inicie el procesamiento.
            <?php else: ?>
    ...
    <div class="flex gap-2">
        <?php if (!$existe_orden_walkin): ?>
        <button type="button" onclick="document.getElementById('manualOrderModal').classList.remove('hidden')" class="bg-white text-orange-600 border border-orange-200 hover:bg-orange-50 px-4 py-2 rounded-lg text-sm font-bold shadow-sm transition-colors">
            Crear orden manual
        </button>
        <?php endif; ?>
...
<div class="flex flex-wrap gap-4 mb-8">
    <a href="<?php echo UrlHelper::url('laboratory'); ?>"
        class="btn-primary-gradient text-white px-6 py-3 rounded-xl font-semibold flex items-center gap-3">
...
<div id="section-pendientes" class="glass-card rounded-2xl shadow-sm border border-amber-50/80 mb-8 overflow-hidden">
    <div class="px-6 py-5 border-b border-gray-100 bg-gradient-to-r from-amber-50/50 to-transparent">
        <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
            ...
            <a href="<?php echo UrlHelper::url('laboratory'); ?>" class="inline-flex items-center gap-2 text-[#007BFF] text-sm font-semibold hover:underline">
                Ir al laboratorio
        ...
        <table id="pendientesTable" class="display w-full">
            ...
            <tbody>
                ...
                <tr class="<?php echo $rowClass; ?>" data-orden-id="<?php echo (int)$o['id']; ?>">
                    ...
                    <td class="py-4 px-2 text-[#495057] text-sm"><?php echo htmlspecialchars($o['examenes'] ?? '-'); ?></td>
                    <td class="py-4 px-2">
                        <span class="inline-flex px-3 py-1 rounded-lg text-xs font-semibold <?php echo estadoBadgeClassLab($o['estado'] ?? 'Pendiente'); ?>">
        ...
        <table id="enProcesoTable" class="display w-full">
            ...
            <tbody>
                ...
                <tr class="border-b border-gray-50 hover:bg-blue-50/30 transition-colors" data-orden-id="<?php echo (int)$o['id']; ?>">
                    ...
                    <td class="py-4 px-2 text-[#495057] text-sm"><?php echo htmlspecialchars($o['examenes'] ?? '-'); ?></td>
                    <td class="py-4 px-2">
                        <span class="inline-flex px-3 py-1 rounded-lg text-xs font-semibold <?php echo estadoBadgeClassLab($o['estado'] ?? 'En proceso'); ?>">
                    ...
                    <td class="py-4 px-2 text-right">
                        <div class="flex flex-wrap gap-2 justify-end">
                            <button type="button" onclick="openResultModal(<?php echo (int)$o['id']; ?>, '<?php echo addslashes(htmlspecialchars(trim(($o['paciente_nombre'] ?? '') . ' ' . ($o['paciente_apellido'] ?? '')))); ?>')" class="btn-success-gradient text-white px-4 py-2 rounded-lg text-sm font-semibold inline-flex items-center gap-2">
        ...
        <table id="completadosTable" class="display w-full">
            ...
            <tbody>
                ...
                <tr class="border-b border-gray-50 hover:bg-emerald-50/30 transition-colors">
                    ...
                    <td class="py-4 px-2 text-[#495057] text-sm"><?php echo htmlspecialchars($o['examenes'] ?? '-'); ?></td>
                    <td class="py-4 px-2">
                        <span class="inline-flex px-3 py-1 rounded-lg text-xs font-semibold <?php echo estadoBadgeClassLab($o['estado'] ?? 'Completada'); ?>">
...
<!-- Modal para cargar resultados (sin redirigir al laboratorio) -->
<div id="resultModal"
    ...
    <div class="glass-card p-4 sm:p-6 md:p-10 rounded-3xl shadow-2xl w-full max-w-lg mx-auto border-white/40 max-h-[90vh] overflow-y-auto">
        ...
        <form action="<?php echo UrlHelper::url('api/laboratory/upload'); ?>" method="POST" enctype="multipart/form-data" class="space-y-6">
            <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars($csrfToken); ?>">
            <input type="hidden" name="orden_id" id="modalOrdenId">
            <input type="hidden" name="redirect_after" value="dashboard_laboratory">
            <div>
                ...
                <textarea name="resultado" id="modalResultadoText" required
                    ...
                    rows="4" placeholder="Ingrese un resumen de los valores..."></textarea>
            </div>
            <div>
                ...
                <p class="text-xs text-[#6C757D] mt-1">Solo se permiten archivos PDF. Obligatorio.</p>
            </div>
            <div class="flex justify-end space-x-3 pt-4">
                ...
                <button type="submit"
                    class="bg-[#28A745] text-white px-8 py-2 rounded-lg font-semibold shadow-md hover:bg-green-700">Guardar resultados</button>
            </div>
        </form>
    </div>
</div>

<!-- Modal para crear orden manual -->
<?php if ($walkin_paciente && !$existe_orden_walkin): ?>
<div id="manualOrderModal" class="hidden fixed inset-0 bg-black bg-opacity-50 backdrop-blur-sm flex items-center justify-center z-50 p-4 transition-all">
    <div class="glass-card p-4 sm:p-6 md:p-10 rounded-3xl shadow-2xl w-full max-w-lg mx-auto border-white/40 max-h-[90vh] overflow-y-auto">
        <h3 class="text-2xl font-bold text-[#007BFF] mb-2">Crear orden manual</h3>
        <p class="text-xs text-[#6C757D] font-medium mb-8 uppercase tracking-wide">Paciente Walk-in: <?php echo htmlspecialchars($walkin_paciente['paciente_nombre'] ?? ''); ?></p>
        <form action="<?php echo UrlHelper::url('api/laboratory/create-walkin'); ?>" method="POST" class="space-y-6">
            <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars($csrfToken); ?>">
            <input type="hidden" name="walkin_id" value="<?php echo $walkin_id; ?>">
            <div>
                <label class="block text-sm font-semibold mb-2">Exámenes solicitados</label>
                <input type="text" name="examenes" required
                    class="w-full px-4 py-2 rounded-lg border focus:ring-2 focus:ring-[#007BFF] outline-none"
                    placeholder="Ej. Hemograma, Perfil lipídico...">
                <p class="text-xs text-[#6C757D] mt-1">Escriba los exámenes separados por coma.</p>
            </div>
            <div>
                <label class="block text-sm font-semibold mb-2">Descripción (Opcional)</label>
                <textarea name="descripcion" class="w-full px-4 py-2 rounded-lg border focus:ring-2 focus:ring-[#007BFF] outline-none"
                    rows="2" placeholder="Notas sobre la orden..."></textarea>
            </div>
            <div class="flex justify-end space-x-3 pt-4">
                <button type="button" onclick="document.getElementById('manualOrderModal').classList.add('hidden')"
                    class="px-6 py-2 rounded-lg border font-semibold text-[#6C757D] hover:bg-gray-50">Cancelar</button>
                <button type="submit"
                    class="bg-[#007BFF] text-white px-8 py-2 rounded-lg font-semibold shadow-md hover:bg-blue-700">Crear orden</button>
            </div>
        </form>
    </div>
</div>
...
var updateStatusUrl = '<?php echo addslashes(UrlHelper::url('api/laboratory/update-status')); ?>';

function openResultModal(id, paciente) {
    document.getElementById('modalOrdenId').value = id;
    document.getElementById('modalPacienteName').innerText = 'Paciente: ' + paciente;
    document.getElementById('modalResultadoText').value = '';
