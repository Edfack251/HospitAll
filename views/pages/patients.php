...
// Control de acceso contextual para médicos (Permiso gestionado por PolicyManager)
...
<div class="flex justify-between items-center mb-8">
    ...
    <?php if ($role !== 'medico'): ?>
        <a href="<?php echo App\Helpers\UrlHelper::url('patients_create'); ?>"
    ...
    <form method="GET" class="flex gap-4">
        <div class="w-48">
            <select name="type" id="idType"
                ...
                <option value="General">Nombre / Teléfono</option>
            </select>
        </div>
        <input type="text" name="search" id="idSearch" value="<?php echo htmlspecialchars($search); ?>"
            placeholder="Buscar paciente..."
            class="flex-1 px-4 py-2 border rounded-lg focus:ring-2 focus:ring-[#007BFF] outline-none" required>
        <button type="submit"
...
<?php if (empty($search)): ?>
    <div class="bg-white p-12 rounded-2xl shadow-sm border border-gray-100 text-center">
        <p class="text-[#6C757D]">Ingrese el nombre, identificación o teléfono para buscar un paciente.</p>
    </div>
<?php elseif (!empty($search) && empty($pacientes)): ?>
    <div class="bg-white p-12 rounded-2xl shadow-sm border border-gray-100 text-center">
        <p class="text-red-500 font-medium">No se encontró ningún paciente que coincida con la búsqueda.</p>
    </div>
<?php else: ?>
            ...
            <tbody>
                <?php foreach ($pacientes as $paciente): ?>
                    <tr class="border-b hover:bg-gray-50 transition-colors">
                        ...
                        <td class="py-4">
                            <?php if ($isMedico): ?>
                                <a href="<?php echo App\Helpers\UrlHelper::url('patient_portal'); ?>?patient_id=<?php echo $paciente['id']; ?>"
                                    class="text-[#28A745] hover:underline font-medium">Ver Historial</a>
                            <?php else: ?>
                                <a href="<?php echo App\Helpers\UrlHelper::url('patients_edit'); ?>?id=<?php echo $paciente['id']; ?>"
