...
- **Verificar remote antes de push**: SIEMPRE ejecutar `git remote -v` y confirmar que la URL del remote coincide con el repositorio destino que el usuario indicó. Nunca asumir que el `origin` actual es el correcto — el workspace puede apuntar a un repo distinto al solicitado.
